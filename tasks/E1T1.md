# E1T1 — Implement /search command and initial place input

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SATB

Create the /search command that starts the guided flow. Prompt the user for a place name and store the input in session state. Implement in src/handlers/E1T1.ts. Write tests in tests/specs/E1T1.json and append /search to tests/commands.json.
