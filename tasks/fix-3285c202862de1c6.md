# fix-3285c202862de1c6 — E6T2: Wrong storage backend — spec requires Postgres or SQLite

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SATB

E6T2 spec states: "Implement persistence for recent queries in a small DB (Postgres or SQLite for prototype)." The implementation uses `RedisSessionStorage` (when `REDIS_URL` is set) or `MemorySessionStorage` (in-memory fallback). Neither is Postgres or SQLite. The spec explicitly names these two databases as the required backend.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-3285c202862de1c6.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-3285c202862de1c6.json` (and any new command `tests/commands/fix-3285c202862de1c6.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-3285c202862de1c6.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-3285c202862de1c6.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
