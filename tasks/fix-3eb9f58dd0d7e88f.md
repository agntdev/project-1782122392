# fix-3eb9f58dd0d7e88f — E4T3 signed S3 URL references a GeoTIFF that is never created

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SATB

The `/download` command produces a correctly-formed AWS SigV4-signed URL — but the target file `outputs/{place}_{composite}_{daterange}_{user}.tif` is never created anywhere. E4T1 only enqueues jobs (never processes them); E4T2 generates a fake PNG instead of calling an imagery pipeline. No code uploads a GeoTIFF to S3. The signed download link will always 404.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-3eb9f58dd0d7e88f.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-3eb9f58dd0d7e88f.json` (and any new command `tests/commands/fix-3eb9f58dd0d7e88f.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-3eb9f58dd0d7e88f.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-3eb9f58dd0d7e88f.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
