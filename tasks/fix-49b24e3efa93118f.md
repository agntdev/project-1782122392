# fix-49b24e3efa93118f — E4T1 queue is write-only — no dequeue or processing logic

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SATB

`/process` enqueues `ProcessingJob` records with literal type `status: "pending"` via `queueStore.write()`. No dequeue, processing, or status-transition code exists anywhere in the repository. The queue fills but jobs are never acted upon. The `ProcessingJob.status` is typed as the literal `"pending"` (not a union), confirming there is no design pathway for completion.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-49b24e3efa93118f.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-49b24e3efa93118f.json` (and any new command `tests/commands/fix-49b24e3efa93118f.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-49b24e3efa93118f.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-49b24e3efa93118f.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
