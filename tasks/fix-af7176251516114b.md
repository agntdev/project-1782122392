# fix-af7176251516114b — TTL is only manually triggerable — no automatic expiration

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SATB

The 30-day TTL specified in E7T2 is only enforced when a user explicitly invokes `/cleanupcache`. There is no automatic/cron-based expiration mechanism. While technically a "TTL" check exists in the cleanup command (`src/handlers/E7T2.ts:7,41`), the design means images can persist indefinitely if `/cleanupcache` is never called.

A real TTL system should expire entries automatically — either on access (checking `storedAt` when retrieving metadata and removing stale entries) or via a periodic background sweep (not just a user-initiated command). Without this, the cache is unbounded in practice.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-af7176251516114b.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-af7176251516114b.json` (and any new command `tests/commands/fix-af7176251516114b.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-af7176251516114b.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-af7176251516114b.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
