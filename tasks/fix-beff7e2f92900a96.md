# fix-beff7e2f92900a96 — E2T2 uses non-null assertion on potentially undefined session start date

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SATB

Line 53: `ctx.session.dateRange = { type: "custom", start: start!, end: text }` where `start = ctx.session.dateRange?.start`. If the session storage is lost or expired between the `awaiting_start` step and the `awaiting_end` step (e.g. Redis flush, in-memory restart), `start` is `undefined`. The `!` assertion forces it into the `DateRange` as `undefined`, silently storing a broken date range instead of handling the missing session gracefully.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-beff7e2f92900a96.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-beff7e2f92900a96.json` (and any new command `tests/commands/fix-beff7e2f92900a96.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-beff7e2f92900a96.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-beff7e2f92900a96.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
