# fix-c42fe426e2601363 — E4T2 generates fabricated preview images — no real satellite data

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SATB

The `generatePreviewPng()` function in `src/handlers/E4T2.ts` creates a synthetic PNG procedurally from `hashString()` of the place/composite/cloud parameters combined with modular arithmetic. No external satellite imagery API (Sentinel, Landsat, Planet, etc.) is called. The implementation contract explicitly forbids this: *"Fabricated data: Math.random(), hardcoded sample arrays, or canned responses standing in for real computed or fetched values."* The preview image must be derived from actual satellite imagery.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-c42fe426e2601363.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-c42fe426e2601363.json` (and any new command `tests/commands/fix-c42fe426e2601363.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-c42fe426e2601363.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-c42fe426e2601363.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
