# fix-d2aa18cd8571e7b5 — E1T2 and E1T3 duplicate Nominatim integration code identically

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SATB

`src/handlers/E1T2.ts` and `src/handlers/E1T3.ts` each define the same `NOMINATIM_BASE` constant, `NominatimResult` interface, `truncate()` helper, and the identical fetch→response.json→slice(0,3)→button-building→`menuKeyboard` pipeline. E1T3 only adds caching on top. This duplication means any fix to the Nominatim integration (URL, User-Agent, error handling, button format) must be applied in both files, risking drift and overlooked fixes.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-d2aa18cd8571e7b5.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-d2aa18cd8571e7b5.json` (and any new command `tests/commands/fix-d2aa18cd8571e7b5.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-d2aa18cd8571e7b5.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-d2aa18cd8571e7b5.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
